...
export class ArticlesSection extends BentoSection {
  ...
  private renderHiddenInput(name: string, value: string) {
    return html`<input type="hidden" name="${name}" value="${value}" />`;
  }

  private renderAuthor(authors: string[]) {
    return html`<li class="bs-results--author li-1">${authors.join('; ')}</li>`;
  }

  private renderPublicationDetails(result: VirgoResult) {
    return html`
