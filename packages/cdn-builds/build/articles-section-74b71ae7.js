...
import { A as ArticlesData } from './ArticlesData-9f3aa85f.js';
...
class ArticlesSection extends BentoSection {
    ...
    renderHiddenInput(name, value) {
        return x `<input type="hidden" name="${name}" value="${value}" />`;
    }
    renderAuthor(authors) {
        return x `<li class="bs-results--author li-1">${authors.join('; ')}</li>`;
    }
    renderPublicationDetails(result) {
      ...
      ${result.publicationType && result.publicationType.length > 0 ? x `<li class="bs-results--format li-1" aria-label="${result.publicationType.join('/')}">${result.publicationType.join('/')}</li>` : ''}
    `;
    }
    render() {
        var _a, _b;
