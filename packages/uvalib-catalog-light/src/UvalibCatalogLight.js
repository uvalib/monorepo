    ...
    return html`
      <uvalib-page nofooter class="coral--light">
        <sp-theme scale="large" color="light">
            <h1>Search</h1>
            <sp-field-label for="name-0">Search Virgo for books, articles, and more.</sp-field-label>
            <sp-textfield id="q" placeholder="Search Virgo for books, articles, and more"></sp-textfield>
            ...
            <div id="results">
              <div id="search-summary" class="summary">
                <div class="query">Showing ${this.totalResultCount} results for:</div>
                <div class="qs">keyword: ${this.lastQuery}</div>
              </div>
              <sp-tabs selected="${this.pools[0].id}" quiet>
                ${this.pools.map(pool => html`
                <sp-tab label="${pool.name} (${ (pool.lastResultCount)? pool.lastResultCount:"0" })" value="${pool.id}"></sp-tab>
                `)}
                ${this.pools.map(pool => html`
                <sp-tab-panel value="${pool.id}">
                  <ul>
                    ${pool.lastResults['group_list'].map(group=>html`
                      <li>${group['record_list'][0].fields.find(field=>field.name.indexOf('title')>-1).value}</li>
                    `)}
                  </ul>
                </sp-tab-panel>
                `)}
