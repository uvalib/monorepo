...
import style from './UvalibCatalogLiteHome.css.js';
...
export class UvalibCatalogLiteHome extends observeState( UvalibAnalyticsMixin(LitElement) ) {
  ...
  firstUpdated() {
    ...
    import('./uvalib-catalog-lite-results.js').then(()=>{
  ...
  _submitSearch(e) {
    ...
    catalogState.pools.uva_library.fetchResults({ rows: 10, keyword: catalogState.rawQueryString }).then(res => {
      ...
      import('./uvalib-catalog-lite-details.js');
      catalogState.pools = {...catalogState.pools, lastTs:new Date() };
      catalogState.hasresults = (catalogState.pools.uva_library.lastResults)? catalogState.pools.uva_library.lastResults.length > 0: false;
...
<div class="home">
  ...
  <div class="search-panel pure-form" ?hidden="${catalogState.focusedItem}">
    ${catalogState.basicSearch? html`
      ...
      <div class="basic-search">
          ...
          >
          <uvalib-button variant="primary" @click="${this._submitSearch}" class="search">Search</uvalib-button>
      </div>
    `:''} 
  </div>
  ...
  <uvalib-catalog-lite-results ?hidden="${catalogState.focusedItem}"></uvalib-catalog-lite-results>
  <uvalib-catalog-lite-details .item="${catalogState.focusedItem}" ?hidden="${!catalogState.focusedItem}"></uvalib-catalog-lite-details>
 </div> 
    `;
  }
