...
export class LibraryWeeklyHours extends LibraryBaseHours {
  // Properties
  @property({ type: Boolean, reflect: true, attribute: "hide-building-hours" }) hideBuildingHours = false;
      ...
      <table ?hidden="${!this.librarySlug}" class="weekly-hours-body-section">
        ...
        <thead>
          <tr>
            ...
            ${
              (this.library && this.library.hours && this.library.hours.rawDates)
                ? Object.entries(this.library.hours.rawDates).map(
                    ([k, v]) => html`
                      <th
                        scope="col"
                        aria-current="false"
                        aria-label="${this.stringDateFormat(k, 0)}"
                        class="${k === this.todayString ? 'today' : ''}"
                      >
                ...
                : html`
                    ${
                      this.library &&
                      Array.isArray(this.library.children) &&
                      this.library.children.length > 0 &&
                      this.library.children[0].hours &&
                      this.library.children[0].hours.rawDates
                        ? Object.entries(
                            this.library.children[0].hours.rawDates
                          ).map(
                            ([k, v]) => html`
        ...
        <tbody>
              ...
              ? html`
                  <tr part="building-hours">
                    ...
                    ${
                      (this.library &&
                        this.library.hours &&
                        this.library.hours.rawDates)
                        ? Object.entries(
                            this.library.hours.rawDates
                          ).map(
                            ([k, v]) => html`
                              <td
                                aria-label="${this.printTimes(v)}"
                                class="${k === this.todayString ? 'today' : ''}"
                              >
                                <div class="date--mobile">
                                  <div class="date-header-day">${this.stringDateFormat(k, 2)}</div>
          ...
          ${
            this.library && this.library.children
              ? this.library.children.map(
                  (l) => html`
                    <tr>
                      <th scope="row" colspan="2" aria-label="${l.title}">
                        ${
                          l.siteLink && l.siteLink.uri
                            ? html`
                                <a
                      ...
                      ${
                        l.hours &&
                        l.hours.rawDates &&
                        l.hours.rawDates.length < 7
                          ? Array.apply(null, Array(7 - l.hours.rawDates.length)).map(
                              () => html`<td></td>`
                            )
                          : ''
                      }
