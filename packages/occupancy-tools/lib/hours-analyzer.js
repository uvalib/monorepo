    ...
    return fetch('https://drupal.lib.virginia.edu/libs?_format=json')
      ...
      .then((d) => {
        if (Array.isArray(d)) {
              ...
              for (const key in locations) {
                ...
                if (
                  ...
                  promises.push(
                    this._firebaseDB
                      .ref(
                        'locations-schemaorg/location/' + key + '/tempClosed',
                      )
                ...
                if (Array.isArray(location.openingHoursSpecification)) {
                  ...
                  if (location.isOpenNow != !!todays)
                    promises.push(
                      this._firebaseDB
                        .ref(
                          'locations-schemaorg/location/' + key + '/isOpenNow',
                        )
                        .set(!!todays)
                        .catch((error) => {
                          this._logError(error);
          ...
          this._logError(
            `Expected an array of libraries from https://drupal.lib.virginia.edu/libs?_format=json but got: ${JSON.stringify(
              d,
            )}`,
          );
        }
