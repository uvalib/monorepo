import { s, i, b, y, Z } from './lit-element-6528530d.js';
...
function unwrapSvg(wrappedSvgObject) {
  ...
  return typeof svgObject === 'function' ? svgObject(y) : svgObject;
}
...
class UvalibModalImageButton extends UvalibAnalyticsMixin(s) {
  static get styles() {
    return [style];
  }
  ...
  render() {
    return (this.alt||this.alt=="")?
      y`<button part="button" class="wrapper" @click="${this.enlarge}" aria-label="Enlarge image of ${this.alt}"><slot></slot></button>`:
      y`<!-- uvalib-image needs an alt attribute even if it is empty! -->`;
  }
