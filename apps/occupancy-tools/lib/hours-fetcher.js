...
export default class HoursFetcher extends OccupancyBase {
  ...
  fetchHours() {
    ...
    return this._firebaseAPIDB
      .ref('libraries')
      .once('value', (libraries) => {
        var hoursKeys = libraries.val().reduce((a, c) => {
          a[c.slug] = c.libcalID;
          return a;
        }, {});
              ...
              if (location['@type'] === 'Library' && hoursKeys[key])
                promises.push(
                  fetch(this.config.libraryHours.libcalURL + hoursKeys[key])
                    .then((res) => res.json())
                    .then((json) => {
                      this._logInfo(`Got hours for ${key}`);
                      if (json.openingHoursSpecification)
                        return this._firebaseDB
                          .ref(
                            `locations-schemaorg/location/${key}/openingHoursSpecification`,
                          )
