...
const TranscriptionResult = z.object({
  ...
  openTags: z.string()      // Context of open tags for next page
});

// Function to transcribe a single-page PDF into TEI XML
export async function transcribePDF(input, pageNumber, priorTags, output) {
...
At the end, state explicitly which tags are open and where the next page should resume.

INPUTS:
- Page number: ${pageNumber}
`;
  if (priorTags) userPrompt += `- Prior page's open tags: ${priorTags}\n`;
...
program
  ...
  .option('-p, --prior-tags <tags>', 'Context of open TEI tags from previous page')
